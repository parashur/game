...
export class GameComponent implements OnInit, OnDestroy {
  ...
  handleKeyboardEvent(event: KeyboardEvent) {
    ...
    if (this.fruitConsumed === this.totalFruits.length) {
      this.code = '';
      return;
    }
    try {
      if (x === 'ArrowLeft') {
        if (this.edgeRgtValues.includes(+fox.parentElement.id - 1)) throw "error"
        console.log("left!", x, +fox.parentElement.id);
        this.keyAction((+fox.parentElement.id - 1).toString())
      }

      if (x === 'ArrowRight') {
        if (this.edgeLftValues.includes(+fox.parentElement.id + 1)) throw "error"
        this.keyAction((+fox.parentElement.id + 1).toString())
      }

      if (this.code === 'ArrowUp') {
        this.keyAction((+fox.parentElement.id - col).toString())
      }
  ...
  move() {
    ...
    const fox = document.getElementById("fox");
    this.clearIntervals = setInterval(() => {
      console.log()
      if (this.gameOver) {
        this.code = '';
        return;
      ...
      try {
      if (!['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown'].includes(this.code)) {
        if (this.edgeLftValues.includes(+fox.parentElement.id + 1)) throw "error"
        this.keyAction((+fox.parentElement.id + 1).toString());
      }
  ...
  getEdgeValues() {
    ...
    for (let i = 1; i < row; i++) {
      const v = col * i - 1;
      edgeRgtValues.push(v);
      edgeLftValues.push(v + 1);
    }
    console.log({ edgeRgtValues });
    this.edgeRgtValues = edgeRgtValues;
    this.edgeLftValues = edgeLftValues;
  }

  ngOnDestroy() {
    if (this.clearIntervals) {
      clearInterval(this.clearIntervals);
      window.clearInterval(this.clearIntervals);
    }
  }
}
